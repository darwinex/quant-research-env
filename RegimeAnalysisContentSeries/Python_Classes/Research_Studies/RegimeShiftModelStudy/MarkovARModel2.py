...
class MarkovAutoRegressiveModels(BaseModel):
    ...
    def _defineModelParameters(self, whichModel, dataDF):
        ...
        if whichModel == 'variance_switching':

            # Define the model:
            self.componentsQty = 2
            self.model = sm.tsa.MarkovRegression(dataDF, 
    ...
    def _fitTheModel(self, whichModel, saveDirectory):
        ...
        for eachAssetName, eachAssetDataFrame in self.PORTFOLIO._portfolioDict.items():
            ...
            if whichModel == 'variance_switching':
                ...
                print(eachAssetDataFrame['SmoothedProbs1'].head())
    ...
    def _saveDataFrames(self, saveDirectory):
        ...
        for eachAssetName, eachAssetDataFrame in self.PORTFOLIO._portfolioDict.items():
            ...
            eachAssetDataFrame.to_csv(saveDirectory + f'/{eachAssetName}_MAR2_DF.csv')

    def _saveModel(self, assetModelName, saveDirectory):

        # Save the model:
        with open(saveDirectory + f'/MarkovAR2_{assetModelName}.pickle', 'wb') as pickle_file:
            pickle.dump(self.model, pickle_file)

    def _loadModel(self, assetModelName, loadDirectory):

        # Load the model:
        with open(loadDirectory + f'/MarkovAR2_{assetModelName}.pickle', 'rb') as pickle_file:
    ...
    def _plotModelOutputFilter(self, saveDirectory='', showIt=False):
        ...
        for eachAssetName, eachAssetDataFrame in self.PORTFOLIO._portfolioDict.items():
            ...
            #ax[2].plot(eachAssetDataFrame.date,eachAssetDataFrame.FilteredProbs3,label='FProbs_Reg3')
            ...
            f1.canvas.set_window_title(f'Probabilities + more data in Markov AR Variance Switching plot for asset <{eachAssetName}>')
            #f1.tight_layout()

            # In PNG:
            plt.savefig(saveDirectory + f'/MarkovAR2_{eachAssetName}.png')
    ...
    def _plotModelOutputSmoother(self, saveDirectory='', showIt=False):
        ...
        for eachAssetName, eachAssetDataFrame in self.PORTFOLIO._portfolioDict.items():
            ...
            #ax[2].plot(eachAssetDataFrame.date,eachAssetDataFrame.SmoothedProbs3,label='SProbs_Reg3')
            #ax[2].set_xlabel('Observations', horizontalalignment='center', verticalalignment='center', fontsize=12, labelpad=20)
            #ax[2].set_ylabel('Probabilities', horizontalalignment='center', verticalalignment='center', fontsize=12, labelpad=20)
            #ax[2].legend(loc='best')
...
if __name__ == "__main__":
    ...
    MARKOV_AR._fitTheModel('variance_switching', None)
