...
class QuantitativeDarwinAnalysis(object):
    ...
    def _getDarwinClosesDataFrame(self, response, darwinSymbol):
        ...
        for eachColumn in DF_CLOSE:
            ...
            # Change negative returns (we cannot go short on DARWINs) for zero values:
        ...
        # Save it to csv:
        DF_RETURNS.to_csv(f'{self.homeStr}/Desktop/Darwinex/quant-research-env/DARWINStrategyContentSeries/DARWINQuantitativeTrading/Data/Daily/returnsData_{darwinSymbol[0]}.csv')
    ...
    def _generateAnalysis(self, historicalDarwinScores):
        ...
        for eachDarwin, eachScoresDf in historicalDarwinScores.items():
            ...
            # Save it to csv:
            dataFrameReturns.to_csv(f'{self.homeStr}/Desktop/Darwinex/quant-research-env/DARWINStrategyContentSeries/DARWINQuantitativeTrading/Data/Daily/scoresData_{eachDarwin}.csv')
    ...
    def _plotInvAttsAndDarwinReturns(self, darwinString, invAttDataframe, showIt=True):
        ...
        for eachAttributeName in invAttDataframe.columns:
            ...
            plt.plot(CLOSE_DF[f'{darwinString}_returns'], label=f'Daily Returns {darwinString}')

            # Settle other stuff:
            plt.grid(linestyle='dotted')
            plt.xlabel('Observations')
            plt.ylabel('Values')
            plt.title(f'Inv Att ({eachAttributeName}) + Daily Returns of Darwin <{darwinString}>')
            plt.legend(loc='best')
            plt.subplots_adjust(left=0.09, bottom=0.20, right=0.94, top=0.90, wspace=0.2, hspace=0)

            # In PNG:
            filePath = f'{self.homeStr}/Desktop/Darwinex/quant-research-env/DARWINStrategyContentSeries/DARWINQuantitativeTrading/Plots/Daily/'
    ...
    def _executeQuantAnalysis(self):
        ...
        FILTERED_DARWINS = ['LVS', 'THA', 'PLF', 'SYO']
